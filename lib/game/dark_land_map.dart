...
import 'package:final_project_ai/random_map/dark_land_generator.dart';
...
class DarkLandMap extends StatelessWidget implements GameListener {
  final LocationSet locationSet;
  final GameController _controller = GameController();
  final double maxHeight;

  DarkLandMap(this.maxHeight, {Key? key, this.locationSet = LocationSet.bottom})
      : super(key: key);
  @override
    ...
    return Stack(
      children: [
        BonfireWidget(
          ...
          map: DarkLandGenerator.map(),
          player: Robot(
            Vector2(
              DarkLandGenerator.playerPosition[1] * tileSize,
              DarkLandGenerator.playerPosition[0] * tileSize,
            ),
          ...
          enemies: DarkLandGenerator.enemies(),
          decorations: DarkLandGenerator.decorations() +
              [
                ExitMapSensor(
                  'ext_bottom',
                  Vector2(
                    DarkLandGenerator.exitPotition[1] * tileSize,
                    DarkLandGenerator.exitPotition[0] * tileSize,
                  ),
  ...
  void _exitMap(String value, BuildContext context) {
    if (value == 'ext_bottom') {
      ...
      context.goTo(DarkLandMap(maxHeight));
    }
  }
